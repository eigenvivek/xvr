#!/bin/bash
#SBATCH --job-name=xvr-finetuned-ljubljana-eval
#SBATCH --output=logs/xvr_finetuned_ljubljana_eval_%A_%a.out
#SBATCH --error=logs/xvr_finetuned_ljubljana_eval_%A_%a.err
#SBATCH --array=0-309
...
CKPTDIR="models/ljubljana/finetuned-ckpts"
...
mkdir -p "results/ljubljana/evaluate/finetuned/$SUBJECT/$CKPT_IDX"
...
xvr register model \
    ...
    -c "$CKPTPATH" \
    -o "results/ljubljana/evaluate/finetuned/$SUBJECT/$CKPT_IDX" \
    --linearize \
    --subtract_background \
