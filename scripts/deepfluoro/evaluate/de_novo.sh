#!/bin/bash
#SBATCH --job-name=xvr-wbct-deepfluoro-de-novo-eval
#SBATCH --output=logs/xvr_wbct_deepfluoro_de_novo_eval_%A_%a.out
#SBATCH --error=logs/xvr_wbct_deepfluoro_de_novo_eval_%A_%a.err
#SBATCH --array=0-185
...
CKPTDIR="models/deepfluoro/de_novo"
...
mkdir -p "results/deepfluoro/evaluate/de_novo/$SUBJECT/$CKPT_IDX"
...
xvr register model \
    ...
    -c "$CKPTPATH" \
    -o "results/deepfluoro/evaluate/de_novo/$SUBJECT/$CKPT_IDX" \
    --crop 100 \
    --linearize \
    --init_only \
    --verbose 0
