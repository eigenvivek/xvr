#!/bin/bash
#SBATCH --job-name=xvr-wbct-deepfluoro-foundation-eval
#SBATCH --output=logs/xvr_wbct_deepfluoro_foundation_eval_%A_%a.out
#SBATCH --error=logs/xvr_wbct_deepfluoro_foundation_eval_%A_%a.err
#SBATCH --array=0-1799
...
CKPT_ARRAY_IDX=$((SLURM_ARRAY_TASK_ID / 6))
SUBJECT_ID=$((SLURM_ARRAY_TASK_ID % 6 + 1))
...
mkdir -p "results/deepfluoro/evaluate/foundation/$SUBJECT/$CKPT_IDX"
...
xvr register model \
    ...
    -c "$CKPTPATH" \
    -o "results/deepfluoro/evaluate/foundation/$SUBJECT/$CKPT_IDX" \
    --crop 100 \
    --linearize \
